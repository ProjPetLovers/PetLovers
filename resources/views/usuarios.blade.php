                ...
                <div class="relative p-8" style="background: linear-gradient(to right, #e3967d, #f28a49);">
                    <div class="text-center text-white">
                        ...
                        <div class="flex items-center justify-center mt-4 space-x-6 text-sm">
                            ...
                            <div class="flex items-center">
                                <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M4.5 12.5c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5S6.83 11 6 11s-1.5.67-1.5 1.5zM9 16c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5S11.33 14.5 10.5 14.5 9 15.17 9 16zm4.5-3c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5-.67-1.5-1.5-1.5-1.5.67-1.5 1.5z"/>
                                </svg>
                                Encontre seu match
                            </div>
                        </div>
    ...
    <div class="p-6">
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
            ...
            <div>
                ...
                <select name="intencao" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm focus:border-[#e3967d] focus:ring-[#e3967d] dark:bg-gray-700 dark:text-white">
                    <option value="">Todas as intenções</option>
                    <option value="Namoro" {{ request('intencao') == 'Namoro' ? 'selected' : '' }}>Namoro</option>
                    <option value="Amizade" {{ request('intencao') == 'Amizade' ? 'selected' : '' }}>Amizade</option>
            ...
            <div>
                ...
                <select name="socializa_com" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm focus:border-[#e3967d] focus:ring-[#e3967d] dark:bg-gray-700 dark:text-white">
                    <option value="">Todas as opções</option>
                    <option value="Adulto" {{ request('socializa_com') == 'Adulto' ? 'selected' : '' }}>Adulto</option>
                    <option value="Criança" {{ request('socializa_com') == 'Criança' ? 'selected' : '' }}>Criança</option>
                    <option value="Outros pets" {{ request('socializa_com') == 'Outros pets' ? 'selected' : '' }}>Outros pets</option>
            ...
            <div>
                ...
                <select name="especie" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm focus:border-[#e3967d] focus:ring-[#e3967d] dark:bg-gray-700 dark:text-white">
                    <option value="">Todas as espécies</option>
                    <option value="Cão" {{ request('especie') == 'Cão' ? 'selected' : '' }}>Cão</option>
                    <option value="Gato" {{ request('especie') == 'Gato' ? 'selected' : '' }}>Gato</option>
                    <option value="Pássaro" {{ request('especie') == 'Pássaro' ? 'selected' : '' }}>Pássaro</option>
                    ...
                    <option value="Réptil" {{ request('especie') == 'Réptil' ? 'selected' : '' }}>Réptil</option>
            ...
            <div class="xl:col-span-6 flex gap-3 mt-4">
                ...
                <a href="{{ route('usuarios') }}" class="inline-flex items-center px-6 py-2 bg-gray-500 hover:bg-gray-600 text-white font-medium rounded-lg transition duration-150 ease-in-out">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="px-4 py-3 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                                <div class="flex items-center justify-between mb-2">
                                    <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center">
                                        <svg class="w-4 h-4 mr-1 text-[#e3967d]" fill="currentColor" viewBox="0 0 24 24">
                                            <path d="M4.5 12.5c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5S6.83 11 6 11s-1.5.67-1.5 1.5zM9 16c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5S11.33 14.5 10.5 14.5 9 15.17 9 16zm4.5-3c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5-.67-1.5-1.5-1.5-1.5.67-1.5 1.5z"/>
                                        </svg>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach($pets->take(4) as $petItem)
                                        <div class="relative group">
                                            @if($petItem->foto)
                                                ...
                                                @endif
                                            @else
                                                <div class="w-12 h-12 rounded-lg bg-gradient-to-br from-[#e3967d] to-[#f28a49] flex items-center justify-center border-2 border-white shadow-sm group-hover:scale-105 transition-transform duration-200">
                                    ...
                                    @if($pets->count() > 4)
                                        <div class="w-12 h-12 rounded-lg bg-gray-300 dark:bg-gray-600 flex items-center justify-center border-2 border-white shadow-sm">
                                ...
                                <div class="mt-2 flex flex-wrap gap-1">
                                    @foreach($pets->take(3) as $petItem)
                                        <span class="text-xs text-gray-600 dark:text-gray-400">
                                            {{ $petItem->nome }}@if(!$loop->last && $pets->count() <= 3),@endif
                                        </span>
                                    @endforeach
                                    @if($pets->count() > 3)
...
@endif
                                ...
                                @if($detalhes?->socializa_com)
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#f28a49] text-white">
                                        <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 24 24">
                                            <path d="M16 4c0-1.11.89-2 2-2s2 .89 2 2-.89 2-2 2-2-.89-2-2zm4 18v-6h2.5l-2.54-7.63c-.24-.74-.9-1.27-1.71-1.27h-5.5c-.8 0-1.47.53-1.71 1.27L9 16h2.5v6h8z"/>
                                        </svg>
                ...
                @empty
                    ...
                    <div class="col-span-full">
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-12 text-center">
                            ...
                            <a href="{{ route('usuarios') }}" class="inline-flex items-center px-6 py-3 bg-[#e3967d] hover:bg-[#8dafa0] text-white font-medium rounded-lg transition duration-150 ease-in-out">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                                </svg>
